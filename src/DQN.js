...
class Memory {
    ...
    sample(batchSize) {
        ...
        for (let i = 0; i < batchSize; ++i) {
            ...
            if (sample) {
                samples.push(this.buffer[this.indices[i]]);
            }
        }
...
class DQN {
    ...
    episode = 0;
    minEpsilon = 0.1;
    targetUpdateInterval = 25;
    epsilonDecay = 1 / 10000;
    ...
    makeSingleModel(inputs, outputs) {
        let optimizer = tf.train.rmsprop(1e-3)//tf.train.adam(1e-4);
        this.outputs = outputs;
    ...
    /*async train() {
        ...
        const states = batch.map(exp => tf.tensor1d(exp.state));
        const actions = batch.map(exp => exp.action);
        const rewards = batch.map(exp => exp.reward);
        const nextStates = batch.map(exp => tf.tensor1d(exp.nextState));
