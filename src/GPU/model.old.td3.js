...
class TD3 {
    ...
    createActorModel() {
        ...
        let output = tf.layers.dense({ units: 32, activation: 'relu', useBias: true }).apply(input)
        output = tf.layers.dense({ units: 64, activation: 'relu', useBias: true }).apply(output)
    ...
    createCriticModel() {
        ...
        let stateOut = tf.layers.dense({ units: 32, activation: 'relu', useBias: true }).apply(stateInput);
        stateOut = tf.layers.dense({ units: 64, activation: 'relu', useBias: true }).apply(stateOut);

        const actionInput = tf.input({ shape: this.outputs });
        let actionOut = tf.layers.dense({ units: 64, activation: 'relu', useBias: true }).apply(actionInput);

        const concat = tf.layers.concatenate().apply([stateOut, actionOut]);

        let out = tf.layers.dense({ units: 128, activation: 'relu', useBias: true }).apply(concat);
