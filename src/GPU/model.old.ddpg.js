...
class RSNorm extends tf.layers.Layer {
    #meanValue = 0;
    #varianceValue = 0;
    #decay = 0;
    ...
    call(inputs, kwargs) {
        return tf.tidy(() => { 
            const inputTensor = inputs instanceof Array ? inputs[0] : inputs;
            const batchMean = tf.mean(inputTensor, 0);
            const batchVariance = tf.moments(inputTensor, 0).variance;

            let mean = tf.tensor(this.#meanValue).mul(this.#decay).add(batchMean.mul(tf.scalar(1).sub(this.#decay)));
            let variance = tf.tensor(this.#varianceValue).mul(this.#decay).add(batchVariance.mul(tf.scalar(1).sub(this.#decay)));

            this.#meanValue = mean.dataSync();
            this.#varianceValue = variance.dataSync();

            return inputTensor.sub(mean).div(tf.sqrt(variance.add(1e-5)));
        });
    }
...
tf.serialization.registerClass(RSNorm);

/*// Bad performance but learns very good
const RFFBlock = ( { units, activation = 'ReLU', input } ) => {
    const relu1 = tf.layers.dense({ units, activation }).apply(input)
    const filter1norm = tf.layers.layerNormalization().apply(relu1);

    const relu2 = tf.layers.dense({ units, activation }).apply(filter1norm)
    const filter2norm = tf.layers.layerNormalization().apply(relu2);
    
    const residual = tf.layers.add().apply([filter1norm, filter2norm]);
    return residual;
};*/

// High performance but learns a little worse
const RFFBlock = ( { units, activation = 'leakyReLU', input } ) => {
    const relu = tf.layers.dense({ units }).apply(input)
    let activationLayer;

    switch(activation){
        case "leakyReLU":
            activationLayer = tf.layers.leakyReLU({ alpha: 0.2 });
            break;
        default:
            activationLayer = tf.layers.activation({ activation });
            break;
    }

    activationLayer = activationLayer.apply(relu);

    const norm = tf.layers.batchNormalization().apply(activationLayer);
    return norm;
...
class DDPG {
    ...
    createActorModel() {
        ...
        let output = input;
        //let output = new RSNorm(1000).apply(input); 
        output = RFFBlock({ units: 128, activation: "leakyReLU", input: output });// makeDenseLayer(output, 64);
        output = RFFBlock({ units: 64, activation: "leakyReLU", input: output });

        output = tf.layers.dense({
            units: this.outputs[0], activation: 'tanh',
            kernelInitializer: tf.initializers.randomUniform({ minval: -0.003, maxval: 0.003 })
        }).apply(output);
    ...
    createCriticModel() {
        const stateInput = tf.input({ shape: this.inputs });
        let stateOut = stateInput;
        //let stateOut = new RSNorm(1000).apply(stateInput); 
        stateOut = RFFBlock({ units: 128, activation: "leakyReLU", input: stateOut });
        stateOut = RFFBlock({ units: 64, activation: "leakyReLU", input: stateOut });

        const actionInput = tf.input({ shape: this.outputs });
        let actionOut = actionInput;
        //let actionOut = new RSNorm(1000).apply(actionInput); 
        actionOut = RFFBlock({ units: 128, activation: "leakyReLU", input: actionOut });

        const concat = tf.layers.concatenate().apply([stateOut, actionOut]);

        let out = RFFBlock({ units: 256, activation: "leakyReLU", input: concat });
        out = RFFBlock({ units: 64, activation: "tanh", input: out });
